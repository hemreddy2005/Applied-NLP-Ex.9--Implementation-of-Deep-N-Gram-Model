...
def create_batch_dataset(lines, vocab, seq_length=100, batch_size=64):
    ...
    # 2) Map text to ids using the provided helper
    #    line_to_tensor(text, vocab) must return a 1-D int tensor of ids
    all_ids = line_to_tensor(single_line_data, vocab)

    # 3) Build a tf.data dataset from ids
    ids_dataset = tf.data.Dataset.from_tensor_slices(all_ids)

    # 4) Window into sequences of length seq_length+1
    data_generator = ids_dataset.batch(seq_length + 1, drop_remainder=True)

    # 5) Split each sequence into input ([:-1]) and target ([1:])
    dataset_xy = data_generator.map(split_input_target)

    # 6) Shuffle, batch into training batches, and prefetch
    dataset = (
        dataset_xy
        .shuffle(BUFFER_SIZE)
...
def compile_model(model):
    ...
    """
    # Define the loss function. Use SparseCategoricalCrossentropy with logits
...
def log_perplexity(preds, target):
    """
    preds: tensor of log-probabilities with shape [batch, time, vocab]
    target: tensor of ids with shape [batch, time]
    Returns: negative mean log-likelihood over non-padding tokens
    """
    PADDING_ID = 1

    # One-hot the targets to match the last dim of preds
    tgt_onehot = tf.one_hot(target, preds.shape[-1])  # [B, T, V]

    # Select the log-prob of the true class for each position
    # preds are already log-probabilities
    log_p = np.sum(tgt_onehot * preds, axis=-1)       # [B, T]

    # Build mask for non-pad positions
    non_pad = 1.0 - np.equal(target, PADDING_ID).astype(np.float32)  # [B, T]

    # Mask out padding
    log_p = log_p * non_pad

    # Average per example over valid tokens, then average over batch
    # sum over time / count of valid tokens per example
    log_ppx_per_ex = np.sum(log_p, axis=-1) / np.sum(non_pad, axis=-1)  # [B]
    log_ppx = np.mean(log_ppx_per_ex)  # scalar
